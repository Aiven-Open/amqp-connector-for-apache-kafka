...
public class RoundTripIT extends KafkaIntegrationTestBase {
    ...
    @Container
    static RabbitMQContainer rabbit = IntegrationTestSetup.rabbitMQContainer();
    ...
    private void setupKafka(AmqpFormat sourceFormat, AmqpFormat sinkFormat, AmqpStrategy sinkStrategy) throws IOException {
    ...
    private Map<String,String> sourceConfig(AmqpFormat amqpFormat) {
        ...
        data.put(
                ...
                AmqpSourceConnector.class.getName());
        data.put(SourceConnectorConfig.KEY_CONVERTER_CLASS_CONFIG, StringConverter.class.getName());
        switch (amqpFormat) {
            case RAW -> data.put(SourceConnectorConfig.VALUE_CONVERTER_CLASS_CONFIG, AmqpConverter.class.getName());
            case NOT_AMQP,
             BODY -> data.put(SourceConnectorConfig.VALUE_CONVERTER_CLASS_CONFIG, StringConverter.class.getName());
    ...
    private Map<String,String> sinkConfig(AmqpFormat amqpFormat, AmqpStrategy strategy) {
        ...
        CommonConfigFragment.setter(data).maxTasks(1);
        data.put(
                SinkConnectorConfig.CONNECTOR_CLASS_CONFIG,
                AmqpSinkConnector.class.getName());
        data.put("topics", getTopic());
        data.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
        switch (amqpFormat) {
            case RAW -> data.put(SinkConnectorConfig.VALUE_CONVERTER_CLASS_CONFIG, AmqpConverter.class.getName());
            case NOT_AMQP,
                 BODY -> data.put(SinkConnectorConfig.VALUE_CONVERTER_CLASS_CONFIG, StringConverter.class.getName());
        }
        return data;
    }
    ...
    void roundTrip(AmqpFormat sourceFormat, AmqpFormat sinkFormat, AmqpStrategy sinkStrategy) throws ClientException, IOException {
...
//                .annotation("response‑address‑cookie", "My cookie".getBytes(StandardCharsets.UTF_8))
...
//                .annotation("unsignedByte", UnsignedByte.valueOf((byte) unsignedByte))
    ...
    private void assertSameRaw(Message<?> actual, Message<?> expected) throws ClientException {
        assertThat(actual.absoluteExpiryTime()).as("absoluteExpiryTime").isEqualTo(expected.absoluteExpiryTime());
        assertThat(actual.to()).as("to").isEqualTo(expected.to());
        assertThat(actual.messageId()).as("messageId").isEqualTo(expected.messageId());
        assertThat(actual.contentEncoding()).as("contentEncoding").isEqualTo(expected.contentEncoding());
    ...
    private void assertSameBody(Message<?> actual, Message<?> expected, AmqpFormat sinkFormat) throws ClientException {
        assertThat(actual.absoluteExpiryTime()).as("absoluteExpiryTime").isEqualTo(expected.absoluteExpiryTime());
        assertThat(actual.to()).as("to").isEqualTo(expected.to());
        assertThat(actual.messageId()).as("messageId").isEqualTo(expected.messageId());
        assertThat(actual.contentEncoding()).as("contentEncoding").isEqualTo(expected.contentEncoding());
...
//        assertThat(actual.userId()).as("userId").isEqualTo(expected.userId());
    ...
    private ProducerRecord<String, String> createProducerRecord(final Message<?> message) throws IOException, ClientException {
        ...
        try (SimpleHeaderConverter simpleConverter = new SimpleHeaderConverter()) {
            for (Header h : cHeaders) {
                recordHeaders.add(h.key(), simpleConverter.fromConnectHeader(null, h.key(), h.schema(), h.value()));
            }
        }

        return new ProducerRecord<>(getTopic(), 0, System.currentTimeMillis(), "nonAmqp", message.body().toString(), recordHeaders);
    }
    ...
    void notAmqpData() throws ClientException, IOException {
        KafkaManager kafkaManager = setupKafka(null, Collections.emptyMap());
        kafkaManager.createTopic(getTopic());
        kafkaManager.configureConnector("Amqp-sink", sinkConfig(AmqpFormat.NOT_AMQP, AmqpStrategy.BODY));
        Message<?> expected = createMessage();
