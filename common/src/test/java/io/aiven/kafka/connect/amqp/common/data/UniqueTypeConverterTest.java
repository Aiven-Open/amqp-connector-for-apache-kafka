...
public class UniqueTypeConverterTest {
    ...
    void ulidTest() {
        ...
        Schema expectedSchema = new SchemaBuilder(Schema.Type.STRING).name(ULID.Value.class.getCanonicalName()).build();
        ...
        Optional<Object> object = underTest.decode(result.get());
        ULID.Value decoded = (ULID.Value) assertThat(object).isPresent().get().isInstanceOf(ULID.Value.class).actual();
        assertThat(decoded).isEqualTo(expectedValue);
