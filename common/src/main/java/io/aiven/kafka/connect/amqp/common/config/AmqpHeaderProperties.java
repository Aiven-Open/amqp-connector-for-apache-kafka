...
public enum AmqpHeaderProperties {
	...
	/**
	 * the currently set 'To' address which indicates the intended destination of
	 * the message.
	 */
	...
	/**
	 * the configured address of the node where replies to this message should be
	 * sent, or null if not set.
	 */
	...
	/**
	 * the assigned content encoding value for the message body section or null if
	 * not set
	 */
	...
	/**
	 * the client-specific id used so that client can send replies to this message
	 * to a specific group.
	 */
	REPLY_TO_GROUP_ID(Schema.OPTIONAL_STRING_SCHEMA),
	/**
	 * For an message being sent this method returns the current state of the
	 * durable flag on the message. For a received message this method returns the
	 * durable flag value at the time of sending (or false if not set) unless the
	 * value is updated after being received by the receiver
	 */
