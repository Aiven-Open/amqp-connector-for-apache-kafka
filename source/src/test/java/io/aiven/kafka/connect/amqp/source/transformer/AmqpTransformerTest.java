...
public class AmqpTransformerTest {
	...
	void byteArrayBody() throws ClientException, JsonProcessingException {
		...
		String expected = ", body="
				+ objectMapper.writeValueAsString("Hello world".getBytes(StandardCharsets.UTF_8)).substring(1);
    ...
    void compressedBody() throws ClientException, IOException {
        ...
        underTest = new AmqpTransformer(sourceCommonConfig);
        byte[] compressedValue = CompressionType.GZIP.compress("Hello world".getBytes(StandardCharsets.UTF_8));
        Message<byte[]> message = ClientMessage.create();
        message.body(compressedValue);

        String expected = ", body=" + objectMapper.writeValueAsString("Hello world".getBytes(StandardCharsets.UTF_8)).substring(1);
