...
public final class AmqpTransformer extends Transformer {
	...
	public static TransformerInfo info() {
		return new TransformerInfo("Amqp", AmqpTransformer.class, TransformerInfo.FEATURE_NONE,
	...
	private Optional<SchemaAndValue> generateValue(AmqpSourceNativeInfo nativeInfo) {
		try {
			...
			if (config.getCompressionType() != CompressionType.NONE) {
				...
				try (InputStream inputStream = config.getCompressionType().decompress(nativeInfo.getInputStream())) {
					IOUtils.copy(inputStream, baos);
				}
				message.body(baos.toByteArray());
			}
			return Optional.of(jsonConverter.toConnectData(null, objectMapper.writeValueAsBytes(message)));
