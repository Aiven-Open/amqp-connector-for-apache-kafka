...
public final class MessageSerializer extends StdSerializer<Message> {
	...
	private void writeObject(final JsonGenerator jgen, final String name, final Object object) throws IOException {
		if (object != null) {
			jgen.writeObjectField(name, object);
		}
	}

	private void writeString(final JsonGenerator jgen, final String name, final String object) throws IOException {
		if (object != null) {
		...
		try {
			for (AmqpHeaderProperties property : AmqpHeaderProperties.values()) {
				switch (property) {
					...
					case CONTENT_ENCODING -> writeString(jgen, property.getSchemaName(), value.contentEncoding());
					case ABSOLUTE_EXPIRY -> jgen.writeNumberField(property.getSchemaName(), value.absoluteExpiryTime());
					case CREATION_TIME -> jgen.writeNumberField(property.getSchemaName(), value.creationTime());
					case GROUP_ID -> writeString(jgen, property.getSchemaName(), value.groupId());
					case GROUP_SEQUENCE -> jgen.writeNumberField(property.getSchemaName(), value.groupSequence());
					case REPLY_TO_GROUP_ID -> writeString(jgen, property.getSchemaName(), value.replyToGroupId());
					case DURABLE -> jgen.writeBooleanField(property.getSchemaName(), value.durable());
					case FIRST_ACQUIRER -> jgen.writeBooleanField(property.getSchemaName(), value.firstAcquirer());
					case DELIVERY_COUNT -> jgen.writeNumberField(property.getSchemaName(), value.deliveryCount());
				}
			}
